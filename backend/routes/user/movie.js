...
app.get("/show", (request, response) => {
    ...
    console.log(movieName, theatreName, showDate);
...
app.get("/moviename/:name", (request, response) => {
    const { name } = request.params;
    const query = "select * from movie where movie_name = ?";

    connection.query(query, [name], (error, result) => {
        if(error) {
            console.log(error);
            response.status(500).json({status : "error", data : "could not get movie"});
        }
        else  {
...
app.post("/addmovie", (request, response) => {
    ...
    const { movie_name, description, duration, genre, release_date, language } = request.body;

    const query = "insert into movie (movie_name, description, duration, genre, release_date, language) values(?, ?, ?, ?, ?, ?)";

    connection.query(query, [movie_name, description, duration, genre, release_date, language], (error, result) => {
        if(error) {
            console.log(error);
            response.status(500).json({status : "error", data : "could not get movie"});
        }
